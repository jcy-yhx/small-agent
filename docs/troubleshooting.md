...
- 验证测试：editable 安装成功，阶段 3 当前 71 个测试通过。
...
- 当前安全读取依赖 POSIX `O_NOFOLLOW`、`O_NONBLOCK` 和相对目录描述符；缺少这些原语的平台会明确拒绝，不会退回到存在竞态窗口的路径式读取。
...
### TR-0007：FIFO 在类型检查前阻塞文件工具

- 首次出现阶段：阶段 3 收尾验收。
- 状态：已解决。
- 表现：工作区内以 `.txt` 或 `.md` 结尾的 FIFO 会让阻塞式 `os.open` 无法返回，程序到不了 `fstat` 的普通文件检查。
- 根因：最终对象先以普通只读模式打开，再检查类型；Agent 步数限制无法中断正在执行的工具。
- 解决方法：最终对象增加 `O_NONBLOCK`，打开后立即对同一描述符执行 `fstat` 并拒绝非普通文件；缺少所需原语的平台安全拒绝。
- 验证测试：真实 `os.mkfifo` 创建命名管道，工具调用立即返回执行错误而不挂起。

### TR-0008：Registry 名称约束与模型调用协议不一致

- 首次出现阶段：阶段 3 收尾验收。
- 状态：已解决。
- 表现：非法名称可以注册并发给模型，但模型返回后无法通过 `ToolCallRequest` 校验。
- 根因：Registry 只检查重名，名称格式规则仅存在于调用请求模型。
- 解决方法：提取共享 `ToolName` 类型；Registry 注册时使用同一约束并拒绝会被空白规范化、格式非法或超过 64 字符的名称。
- 验证测试：空名称、前导空格、连字符、大写开头和超长名称均在注册阶段拒绝。

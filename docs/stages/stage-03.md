...
5. 让 CLI 创建唯一 Registry，同时注入 Client 与 Runner。
6. 补齐确定性测试和五类真实路由验收。
...
- `SiliconFlowLLMClient(..., tool_definitions=...)`：模型能力来自同一 Registry。
- `AgentRunner(..., registry=...)`：执行不再依赖具体工具类。
...
- 自动化：`.venv/bin/python -m pytest`，`62 passed in 0.67s`。
- Registry：注册、重复名、definition、未知工具和 Echo 扩展测试通过。
- 文件安全：绝对路径、`..`、隐藏文件、错误后缀、符号链接逃逸和超限均拒绝。
...
- 未知工具：确认工具已在默认 Registry 注册，且 Client 与 Runner 使用同一个 Registry。
...
文件读取让我再次看到“只读”也不等于没风险。我把范围限制为启动工作区内、非隐藏的 `.txt/.md`、UTF-8、最大 64 KiB，并在解析符号链接后重新检查边界。`.env`、绝对路径、`..` 和链接逃逸都会被程序拒绝。

62 个离线测试通过后，我又跑了五个真实案例：四种工具全部选对，普通 Python 知识题则没有乱用工具，路由 5/5。下一阶段才会面对真正危险的写文件和 Shell，并加入权限与人工确认。#AIAgent #ToolCalling #Python #Agent开发 #从零学习
